	...
	async def receive_json(self, content):
		...
		try:
			if command == "get_general_notifications":
				...
				await mark_notifications_read(self.scope["user"],content.get("timestamp",None))
...
def refresh_general_notifications(user, oldest_timestamp, newest_timestamp):
	...
	if user.is_authenticated:
		...
		content_type__in=[friend_request_ct, friend_list_ct], timestamp__gte=oldest_ts, timestamp__lte=newest_ts).order_by('-timestamp')
...
@database_sync_to_async
def mark_notifications_read(user,timestamp):
	"""
	...
	if user.is_authenticated:
		...
		friend_list_ct = ContentType.objects.get_for_model(FriendList)
		print(timestamp)
		notifications = Notification.objects.filter(target=user, content_type__in=[friend_request_ct, friend_list_ct], timestamp__gte=timestamp, read=False).order_by('-timestamp')
		# notifications = Notification.objects.filter(target=user)
		if notifications:
